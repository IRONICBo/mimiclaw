...
esp_err_t llm_claude_init(void)
{
    ...
    if (s_api_key[0]) {
        ESP_LOGI(TAG, "Claude provider initialized (model: %s)", s_model);
    } else {
        ESP_LOGW(TAG, "No Claude API key configured");
    }
    return ESP_OK;
}

/* HTTP call using direct method */
static esp_err_t claude_http_direct(const char *post_data, resp_buf_t *rb, int *out_status)
{
...
/* HTTP call via proxy */
static esp_err_t claude_http_via_proxy(const char *post_data, resp_buf_t *rb, int *out_status)
{
...
static esp_err_t claude_http_call(const char *post_data, resp_buf_t *rb, int *out_status)
{
    if (http_proxy_is_enabled()) {
        return claude_http_via_proxy(post_data, rb, out_status);
    } else {
        return claude_http_direct(post_data, rb, out_status);
    }
}
...
esp_err_t llm_claude_chat(const char *system_prompt, const char *messages_json,
                          char *response_buf, size_t buf_size)
{
    if (s_api_key[0] == '\0') {
        snprintf(response_buf, buf_size, "Error: No Claude API key configured");
    ...
    int status = 0;
    esp_err_t err = claude_http_call(post_data, &rb, &status);
    free(post_data);
...
/* Chat with tools */
esp_err_t llm_claude_chat_tools(const char *system_prompt,
                                cJSON *messages,
...
/* Set API key */
esp_err_t llm_claude_set_api_key(const char *api_key)
{
    nvs_handle_t nvs;
    ...
    ESP_LOGI(TAG, "Claude API key saved");
    return ESP_OK;
}

/* Set model */
esp_err_t llm_claude_set_model(const char *model)
{
    nvs_handle_t nvs;
    ESP_ERROR_CHECK(nvs_open(MIMI_NVS_LLM, NVS_READWRITE, &nvs));
    ...
    ESP_LOGI(TAG, "Claude model set to: %s", s_model);
    return ESP_OK;
}

/* Get provider name */
const char* llm_claude_get_name(void)
{
    return "claude";
}

/* Provider interface */
static const llm_provider_t s_claude_provider = {
    .type = LLM_PROVIDER_CLAUDE,
    .init = llm_claude_init,
    .chat = llm_claude_chat,
    .chat_tools = llm_claude_chat_tools,
    .set_api_key = llm_claude_set_api_key,
    .set_model = llm_claude_set_model,
    .get_name = llm_claude_get_name,
};

const llm_provider_t* llm_get_claude_provider(void)
{
    return &s_claude_provider;
}
